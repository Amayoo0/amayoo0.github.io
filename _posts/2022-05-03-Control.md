...
if __name__ == "__main__":
    ...
    if(len(sys.argv) > 1 and sys.argv[1] == 'phpCommand'):
        ...
        injection()
```
Examine databases, tables and values. The following injection will report the hash of a posible users. Crack them in (https://crackstation.net) and save the results: 
```
...
### Privilege Scalation
Fists of all, look at the privilege of your user: `whoami /priv`. But as you can see, there is not anyone we know. 
...
You can edit this service. It is possible to change the `ImagePath` of it and select your binary:
1. See the features: 
``` powershell
...
```
4. You're right: Start the service.
``` powershell
